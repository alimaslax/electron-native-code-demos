  ...
  static Napi::Object Init(Napi::Env env, Napi::Object exports) {
    Napi::Function func = DefineClass(
        ...
        {InstanceMethod("helloWorld", &SwiftAddon::HelloWorld),
         InstanceMethod("searchApplications", &SwiftAddon::SearchApplications),
         InstanceMethod("launchApplication", &SwiftAddon::LaunchApplication),
  ...
  SwiftAddon(const Napi::CallbackInfo &info)
    ...
    napi_status status = napi_create_threadsafe_function(
        ...
        [](napi_env env, napi_value js_callback, void *context, void *data) {
          ...
          try {
            auto callback = addon->callbacks.Value()
                                .Get(callbackData->eventType)
                                .As<Napi::Function>();
            if (callback.IsFunction()) {
              callback.Call(
                  addon->emitter.Value(),
                  {Napi::String::New(napi_env, callbackData->payload)});
            }
          } catch (...) {
          }
      ...
      return ^(NSString *payload) {
        if (tsfn_ != nullptr) {
          auto *data = new CallbackData{
              eventType, std::string([payload UTF8String]), this};
...
private:
  ...
  Napi::Value SearchApplications(const Napi::CallbackInfo &info) {
    Napi::Env env = info.Env();
    ...
    NSString *result = [SwiftBridge searchApplications:nsQuery];
  ...
  Napi::Value LaunchApplication(const Napi::CallbackInfo &info) {
    ...
    [SwiftBridge launchApplication:nsId];
  ...
  Napi::Value On(const Napi::CallbackInfo &info) {
    ...
    if (info.Length() < 2 || !info[0].IsString() || !info[1].IsFunction()) {
      Napi::TypeError::New(env, "Expected (string, function) arguments")
          .ThrowAsJavaScriptException();
      return env.Undefined();
    }

    callbacks.Value().Set(info[0].As<Napi::String>(),
